...
void test_primescope() {
  ...
  #define EV_LLC MAX_EXTENSION  // Use MAX_EXTENSION instead of LLC_WAYS to accommodate larger eviction sets
...
void configure_thresholds(
  ...
  *thrLLC = timing[0][(int)0.90*THRESHOLD_TEST_COUNT];  // Use 90th percentile for more conservative threshold
  *thrRAM = timing[1][(int)0.50*THRESHOLD_TEST_COUNT];
  *thrL1  = timing[2][(int)0.10*THRESHOLD_TEST_COUNT];
  *thrDET = (*thrLLC + (*thrRAM))/2;  // Midpoint between LLC and RAM

  printf("Thresholds: L1=%d, LLC=%d, RAM=%d, DET=%d\n", *thrL1, *thrLLC, *thrRAM, *thrDET);
}
