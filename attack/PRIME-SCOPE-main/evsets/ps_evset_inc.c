  ...
  while (list_len<len) {
    ...
    while (try_guesses) {
      ...
      if (time>threshold-20) {
        ...
        if (list_len <= 3) {
          printf("DEBUG: Found congruent address #%d, time=%d, threshold=%d\n",
                 list_len, time, threshold);
        }
      ...
      if (guess_index>=pool_size-1) {
        ...
        if (++counter_attempt>=MAX_ATTEMPT){ // If too many wrap-arounds, return with fail
          printf("DEBUG: Failed construction - counter_attempt=%d, list_len=%d/%d, pool_size=%d\n",
                 counter_attempt, list_len, len, pool_size);
