  ...
  for (size_t i = 0; i < 64*1024*1024-4096; i += 4096) {
    ...
    if (phys1 != phys2 && in_same_cache_set(phys1, phys2, -1)) {
      if (found < ADDR_COUNT) {
        ...
        printf("Warning: Eviction set is full.\n");
        break;
      }
    }
  }
...
size_t onlyreload(void* addr) // cached
{
  size_t time, delta;
  time = rdtsc();
  for (size_t i = 1; i < PROBE_COUNT && i < found - 2; ++i) {
    *faddrs[i];
    *faddrs[i+1];
    *faddrs[i+2];
    *faddrs[i];
...
int main(int argc, char** argv)
{
  ...
  if (found < PROBE_COUNT + 2) {
    printf("Error: Not enough addresses in the eviction set.\n");
    return 1;
  }
  printf("start\n");
  maccess(array + 2*1024);
